...
public class MonthAdapter extends RecyclerView.Adapter<MonthAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(MonthAdapter.ViewHolder holder, int position) {
        ...
        if (!isPositionHeader(position)) {
            if (position - 7 - mFirstDayOfWeek >= 0) {
                holder.itemDay.setClickable(true);
                holder.itemDay.setFocusable(true);
                TypedValue typedValue = new TypedValue();
                mContext.getTheme().resolveAttribute(android.R.attr.selectableItemBackgroundBorderless, typedValue, true);
                if (typedValue.resourceId != 0) {
                    holder.itemDay.setBackgroundResource(typedValue.resourceId);
                } else {
                    holder.itemDay.setBackgroundColor(typedValue.data);
                }
